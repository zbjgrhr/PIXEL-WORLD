...
export function createFallbackGameSpec(
  ...
  const backgroundStory = section(
    prompt,
    ['background story', 'story', '背景故事'],
    `Long ago, ${themeName.trim() || 'this pixel world'} was protected by the power now sealed inside the ${collectibleAppearance}. When the ${bossAppearance} shattered that balance and filled the roads with hostile creatures, the surviving settlements lost contact with one another. A lone adventurer carrying ${weaponAppearance} must cross every region, recover the scattered energy, and reopen the ancient gateways. Each conquered battlefield brings the hero closer to the final guardian and to the truth behind the world's collapse.`,
  )
