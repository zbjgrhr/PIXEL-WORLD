...
当前版本在传统单次提示词优化之外，新增本地优先的多 Agent 工作台。Director 建立创作简报；叙事、玩法和美术 Agent 并行策划；关卡 Agent 组织逐关内容；Integrator 合并为 GameSpec V3；一致性评审与 Engine QA 并行检查；Revision 最多执行两轮修正；Asset Coordinator 只在规格通过后整理素材提示词。所有文字任务锁定同一模型，图片任务另行锁定同一图片模型。

Agent 只返回经过字段白名单和 GameSpec 标准化的数据，不生成或执行任意 JavaScript。生图前设置人工审批节点；Visual QA、Playtest 和 Publisher 只生成检查报告，实际素材队列、游戏运行与 ZIP 导出仍由确定性程序负责。任务记录保存到 IndexedDB，API Key 仅保存在浏览器会话中。完整数据与流程说明见 `docs/AGENT_CLUSTER.md`。
