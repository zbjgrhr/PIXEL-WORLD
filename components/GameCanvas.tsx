...
import type { AnimationClipPose, AnimationPose, AssetCategory, AssetDefinition, AssetType, GameCanvasProps, ObstacleData } from '@/types'
...
function plannedSprite(
  ...
  const animation = asset?.kind === 'spriteSheet' && asset.animation ? normalizeAnimationSpec(asset.animation) : undefined
  const clipPose: AnimationClipPose = state === 'attack' ? 'meleeAttack' : state
  const clip = animation?.layoutVersion === 3 ? animation.clips?.[clipPose] : undefined
  const spriteUrl = clip?.url || (animation?.layoutVersion === 3 ? animation.clips?.idle?.url : undefined) || url
  if (!spriteUrl) return fallback
  if (!animation) {
    return <img src={spriteUrl} alt={alt} draggable={false} style={{ ...style, imageRendering: 'pixelated', userSelect: 'none', pointerEvents: 'none' }} />
  }
  if (animation.layoutVersion === 3) {
    const frameCount = clip?.url ? clip.frameCount : 1
    const elapsed = Math.max(0, timeMs - stateStartedAt)
    const oneShot = clipPose === 'meleeAttack' || clipPose === 'rangedAttack' || clipPose === 'death'
    const frame = clipPose === 'idle' || clipPose === 'hit'
      ? 0
      : clipPose === 'jump'
        ? Math.min(frameCount - 1, jumpFrame)
        : oneShot
          ? Math.min(frameCount - 1, Math.floor(elapsed / Math.max(1, stateUntil - stateStartedAt) * frameCount))
          : Math.floor(elapsed / (1000 / (clip?.fps || animation.fps))) % frameCount
...
const GameCanvas: React.FC<GameCanvasProps> = ({ onBackToMenu }) => {
  ...
  const meleeAttack = useCallback(() => {
    ...
    setActorAction(player, 'meleeAttack', now, ATTACK_POSE_MS)
    const hitbox = {
  ...
  const rangedAttack = useCallback(() => {
    ...
    setActorAction(player, 'rangedAttack', now, ATTACK_POSE_MS)
    ...
    const loop = (realNow: number) => {
      ...
      if (modeRef.current === 'playing' && player) {
        ...
        if (player.health <= 0) {
          ...
          const reactionLocked = (player.action === 'meleeAttack' || player.action === 'rangedAttack' || player.action === 'hit') && now < player.actionUntil
        ...
        enemiesRef.current.forEach((enemy) => {
          ...
          const reactionLocked = (enemy.action === 'meleeAttack' || enemy.action === 'rangedAttack' || enemy.action === 'hit') && now < enemy.actionUntil
          ...
          if (!reactionLocked && enemy.ranged && Math.abs(distance) < 520 && now - enemy.lastShotAt > shotDelay) {
            ...
            setActorAction(enemy, 'rangedAttack', now, ATTACK_POSE_MS)
            projectilesRef.current.push({
            ...
            enemy.lastShotAt = now
            setActorAction(enemy, 'meleeAttack', now, ATTACK_POSE_MS)
        ...
        <div style={{ position: 'absolute', width: worldWidth, height: stageHeight, transform: `translateX(${-cameraX}px)`, willChange: 'transform', filter: filterStyle, zIndex: 2 }}>
          ...
          {enemiesRef.current.map((enemy) => {
            ...
            <div key={enemy.id} data-testid={enemy.isBoss ? 'game-boss' : 'game-enemy'} data-action={enemy.action} style={{ position: 'absolute', left: enemy.x, top: enemy.y, width: enemy.width, height: enemy.height }}>
              ...
              {plannedSprite(
                ...
                enemy.vy < 0 || enemy.y < enemy.baseY ? 0 : 1,
              )}
            </div>
          )})}
          ...
          {player && (
            <div data-testid="game-player" data-action={player.action} style={{
              ...
              {plannedSprite(heroAsset, assets.character, player.action, renderTick, player.actionStartedAt, player.actionUntil, spec.hero.name, { width: '100%', height: '100%', objectFit: 'contain', transform: `scaleX(${player.facing})` }, fallbackSprite('HERO', '#2664c9', 54), player.vy < 0 ? 0 : 1)}
              <div style={{ position: 'absolute', left: player.facing === 1 ? 34 : -14, top: 25, width: 34, height: 20 }}>
                {renderImage(player.action === 'rangedAttack' ? rangedWeaponAsset?.url || assets.rangedWeapon : player.action === 'meleeAttack' ? meleeWeaponAsset?.url || assets.weapon : '', spec.weapon.name, { width: '100%', height: '100%', objectFit: 'contain', transform: `scaleX(${player.facing}) rotate(${player.facing === 1 ? -10 : 10}deg)` }, null)}
              </div>
            </div>
          )}
